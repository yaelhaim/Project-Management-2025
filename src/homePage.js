      ...
      productElement.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div style="flex: 1; display: flex; align-items: center;">
                        <a href="productPage.HTML?productId=${
                          product.id
                        }" class="product-name" style="font-weight: bold;">${
        product.name
      }</a>
                        ${
        ...
        <a href="#" class="product-category" data-category-id="${
          product.category_id
        }" style="font-weight: bold; display: block; margin-top: 5px;">Category: ${getCategoryNameById(
        product.category_id
      )}</a>
      ...
      productElement.innerHTML = `
        ...
        <a href="#" class="product-category" data-category-id="${
          product.category_id
            ...
            <span style="font-weight: bold;">Category:</span> ${getCategoryNameById(
              product.category_id
            )}
        </a>
        <img src="${product.image_url}" alt="${
...
const sortElements = (function () {
  ...
  const sortByPopularity = () => {
    productsData.filteredProducts.sort(
      (p1, p2) => p2.number_of_purchase - p1.number_of_purchases
    ); //rating
    updateProductList(productsData.filteredProducts);
    presentSort.classList.add("presentSort_hidden");
  };
  const sortByRecency = () => {
    productsData.filteredProducts.sort((p1, p2) =>
      p2.created_at.localeCompare(p1.created_at)
    );
    updateProductList(productsData.filteredProducts);
    presentSort.classList.add("presentSort_hidden");
  };
...
const filterHandlers = (function () {
  const validateCharacters = (input, p) => {
    ...
    try {
      ...
      if (
        ...
        // validate value is positive and up to 10 characters
        throw new Exception();
      }
      p.innerHTML = "&nbsp;";
      ...
      // on failure present an error meesgase
  ...
  const validateTo = () => {
    const toNum = validateCharacters(
      filterElements.to,
      filterElements.toMessage
    );
    const fromNum = validateCharacters(
      filterElements.from,
      filterElements.fromMessage
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const shoppingCartIcon = document.querySelector(
    ".material-symbols-outlined.shopping_cart-icon"
  );
  ...
  document.getElementById("filterByDiv").addEventListener("click", (event) => {
    document
      .getElementById("presentFilter")
      .classList.remove("presentFilter_hidden");
    event.stopImmediatePropagation();
  ...
  filterElements.btnApply.addEventListener(
    "click",
    filterHandlers.FilterByPriceAndRating
  );

  sortElements.btnPopularity.addEventListener(
    "click",
    sortElements.sortByPopularity
  );
